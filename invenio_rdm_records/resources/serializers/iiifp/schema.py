...
class IIIFPresiSchema(Schema):
    ...
    def imgUriBase(self, obj):
        ...
        # TODO: should be in the record.links.iiif_base
        return f"{current_app.config['SITE_API_URL']}/iiif/v2/{obj['id']}"

    def presiUriBase(self, obj):
        """Base for presentation API URIs."""
        # TODO: should this be API URL?
        return f"{current_app.config['SITE_API_URL']}/records/{obj['id']}"

    def make_manifest_id(self, obj):
        """Create maniest id."""
        # TODO: get from records.links to avoid hard-coding. Also this ensures
        # that the serializer does not need to know if it's a draft or record.
    ...
    def make_metadata(self, obj):
        ...
        # TODO: more fields to show in metadata?
        m = obj["metadata"]
        return [
                {
    ...
    def make_sequence(self, obj):
        ...
        # TODO: needs to serialize files in the record to avoid accessing
        # internal _record object. service.read/read_draft should have an
        # option to ask for files to be included so that
        # resource.read_iiif_manifest() can  ask to have the files available
        # as well. Alternatively, we need the result item to have a method to
        # get the files.
